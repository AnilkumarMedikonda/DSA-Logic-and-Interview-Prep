...
// MARK: - Problem
// 81. Permutation in String (LeetCode 567)
//
// Given two strings s1 and s2, return true if s2 contains a
// permutation of s1 as a substring, otherwise false.
//
// A permutation = any rearrangement of s1's characters.
// So we are NOT searching for specific strings — we are checking:
// does any window of length s1.count in s2 have the exact same
// character frequency map as s1?
//
// Example 1: s1 = "ab", s2 = "eidbaooo" -> true  ("ba" is a window)
// Example 2: s1 = "ab", s2 = "eidboaoo" -> false (a, b exist but not adjacent)
//
// Constraints: 1 <= s1.count, s2.count <= 10^4, lowercase English letters.
...
let tests: [(s1: String, s2: String, expected: Bool)] = [
    ("ab", "eidbaooo", true),    // "ba" window exists
    ("ab", "eidboaoo", false),   // a, b never adjacent
    ("a", "a", true),            // single char, exact match
    ("a", "b", false),           // single char, no match
    ("abc", "ab", false),        // pattern longer than search string
    ("adc", "dcda", true),       // "cda" is a permutation of "adc"
    ("hello", "ooolleoooleh", false), // chars exist, frequencies never align in one window
    ("aab", "eidbaaboo", true)   // duplicate chars in pattern: "aab" window
]

for test in tests {
    let bruteResult = checkInclusionBruteForce(test.s1, test.s2)
    let optimisedResult = checkInclusionOptimised(test.s1, test.s2)
    let bruteStatus = bruteResult == test.expected ? "PASS" : "FAIL"
    let optimisedStatus = optimisedResult == test.expected ? "PASS" : "FAIL"
    print("s1: \(test.s1), s2: \(test.s2) | brute: \(bruteResult) [\(bruteStatus)] | optimised: \(optimisedResult) [\(optimisedStatus)]")
}

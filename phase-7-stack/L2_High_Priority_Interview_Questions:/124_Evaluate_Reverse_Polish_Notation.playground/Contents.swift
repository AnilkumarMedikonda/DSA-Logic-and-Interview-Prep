// 124_Evaluate_Reverse_Polish_Notation
// LC 150 — Medium — Stack

// MARK: - Problem
/*
 Given an array of strings `tokens` representing an arithmetic expression
 in Reverse Polish Notation (postfix), evaluate it and return the result.

 - Operators: + - * /
 - Operands can be negative integers (e.g. "-11")
 - Division truncates toward zero (6 / -132 = 0, -7 / 2 = -3)
 - Expression is always valid, no division by zero

 Example: ["2","1","+","3","*"] → ((2 + 1) * 3) = 9

 Why stack: an operator always applies to the two MOST RECENTLY seen
 operands — "most recent" is exactly LIFO.
 */
